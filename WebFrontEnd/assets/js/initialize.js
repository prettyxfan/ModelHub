...
function dealAnalysisData(s){
    ...
    for(var x in T){
        var text = T[x];
        $("#table").append("<option value='"+text+"'>"+text+"</option>");
...
$("#table").change(function(){
    ...
    for(var x in T){
        if(T[x] == tableName){
            $("#x").append("<option value='"+X[x][0]+"'>"+X[x][0]+"</option>");
            $("#x").append("<option value='"+Y1[x][0]+"'>"+Y1[x][0]+"</option>");
            $("#x").append("<option value='"+Y2[x][0]+"'>"+Y2[x][0]+"</option>");
            $("#y1").append("<option value='"+X[x][0]+"'>"+X[x][0]+"</option>");
            $("#y1").append("<option value='"+Y1[x][0]+"'>"+Y1[x][0]+"</option>");
            $("#y1").append("<option value='"+Y2[x][0]+"'>"+Y2[x][0]+"</option>");
            $("#y2").append("<option value='"+X[x][0]+"'>"+X[x][0]+"</option>");
            $("#y2").append("<option value='"+Y1[x][0]+"'>"+Y1[x][0]+"</option>");
            $("#y2").append("<option value='"+Y2[x][0]+"'>"+Y2[x][0]+"</option>");
        }
    }
...
function displayChart(){
    ...
    for(var x in T){
        if(T[x] == tableName){
            if(x1name == X[x][0]) x1 = X[x].splice(1);
            else if(x1name == Y1[x][0]) x1 = Y1[x].splice(1);
            else if(x1name == Y2[x][0]) x1 = Y2[x].splice(1);

            if(y1name == X[x][0]) y1 = X[x].splice(1);
            else if(y1name == Y1[x][0]) y1 = Y1[x].splice(1);
            else if(y1name == Y2[x][0]) y1 = Y2[x].splice(1);

            if(y2name == X[x][0]) y2 = X[x].splice(1);
            else if(y2name == Y1[x][0]) y2 = Y1[x].splice(1);
            else if(y2name == Y2[x][0]) y2 = Y2[x].splice(1);

        }
    }

    $("#canvasDialog").modal('show');
    var rg = document.getElementsByName("chart");
    for(var i=0;i<rg.length;i++){
        if(rg[i].checked && rg[i].value == "Line"){
            line(x1,y1,y2);
        }
        else if(rg[i].checked && rg[i].value == "Bar"){
            bar(x1,y1,y2);
        }
    }
}
